...
void tReglasSudoku::clear_blocked_values(int f, int c, int v) {
	...
	for (int i = 0; i < dim; i++) {
		valores_celda[f][i][v - 1].celdas_que_afectan--;
		if (valores_celda[f][i][v - 1].celdas_que_afectan == 0 && !valores_celda[f][i][v - 1].posible) {
			cuantas_celdas[posible_values(f, i) - 1]--;
			valores_celda[f][i][v - 1].posible = true;
			if (posible_values(f, i)>0)cuantas_celdas[posible_values(f, i) - 1]++;
	...
	for (int i = 0; i < dim; i++) {
		valores_celda[i][c][v - 1].celdas_que_afectan--;
		if (valores_celda[i][c][v - 1].celdas_que_afectan == 0&&!valores_celda[i][c][v - 1].posible) {
			cuantas_celdas[posible_values(i, c) - 1]--;
			valores_celda[i][c][v - 1].posible = true;
			if (posible_values(i, c) > 0)cuantas_celdas[posible_values(i, c) - 1]++;
	...
	for (int i = 0; i < hdim; i++) {
		for (int j = 0; j < hdim; j++) {
			valores_celda[fa + i][ca + j][v - 1].celdas_que_afectan--;
			if (valores_celda[fa + i][ca + j][v - 1].celdas_que_afectan == 0&&!valores_celda[fa + i][ca + j][v - 1].posible) {
				cuantas_celdas[posible_values(fa + i, ca + j) - 1]--;
				valores_celda[fa + i][ca + j][v - 1].posible = true;
				if (posible_values(fa + i, ca + j) > 0)cuantas_celdas[posible_values(fa + i, ca + j) - 1]++;
...
bool tReglasSudoku::set_value(int f, int c, tCelda celda) {
	...
	if (0<=c&&0<=f&&f < dim && c < dim && is_posible_value(f, c, celda.get_value())) {


		if (get_celda(f, c).is_empty()) { cuantas_celdas[posible_values(f, c) - 1]--; }
