...
using namespace std; // ya incluido
...
void ask(tReglasSudoku& const rTab) { // muestra el tablero y el menú, y si el sudoku tiene celdas bloqueadas, muestra cuales son
...
bool resolver_sudoku(tReglasSudoku& reglas, int fila, int columna) { // funcion recursiva para resolver el sudoku
...
void download_paths(ifstream& arc, tListaSudoku& list) { // descarga la lista de sudokus originales a partir de un archivo con sus rutas
...
void download_sudoku(ifstream& arc, tReglasSudoku& rg) { // descarga un sudoku original, a partir de un archivo con su ruta y añade sus celdas ocupadas para luego poder jugar con él
...
void download_lista(ifstream& arc, tListaSudoku& list) { // repeticion de download_sudoku
...
void load_list(string PA, tListaSudoku& const ls) { // guarda los sudokus modificados para poder jugar mas tarde
	
	ofstream arc; arc.open(PA, ios::trunc);
	int h, n = ls.dame_num_elems();
	...
	for (int a = 0; a < n; a++) {
		...
		for (int i = 0; i < h; i++) {
			for (int j = 0; j < h; j++) {
				...
				if (celda.is_taken()) arc << i << ' ' << j << ' ' << celda.get_value() << '\n';
			}
		}
		arc << -1;
	}
...
void playing(tReglasSudoku& rTab) { // función principal del juego, muestra el tablero y el menú, y gestiona las opciones elegidas por el usuario
	...
	do {
		if (cin >> option) {
			...
			case 1:
				...
				cout << "Valor: ";
				cin >> valor;
				i--; j--; // para adaptar a la numeracion del usuario, que empieza en 1, y la del programa, que empieza en 0
				
				if (i >= rTab.get_dimension() || j >= rTab.get_dimension() || i < 0 || j < 0) {

					cout << RED << "Error, posicion fuera de rango\n" << RESET;
				}
				else if (valor <= 0 || valor > rTab.get_dimension()) {

					cout << RED << "Error, valor fuera de rango\n" << RESET;
				}
				else if (rTab.get_celda(i, j).is_original()) {

					cout << RED << "Error, celda original\n" << RESET;
				}
				else if (not rTab.get_celda(i, j).is_empty()) {

					cout << RED << "Error, celda ocupada\n" << RESET;
				}
				else if (rTab.is_posible_value(i, j, valor)) { // incluimos la antigua comprobacion de is_posible_value
...
int main() {
	...
	while (salir != 'A') {
		...
		if (salir == 'N') {
			...
			if (!rTab.finish())	games_list.insertar(rTab);
		}
		else if (salir == 'C') {
	...
	cout << YELLOW << "Gracias por jugar. Hasta la proxima!!\n" << RESET;
