...
public:
	...
	Celda get_celda(int f, int c) const; // devuelve la celda en la posición (f,c)
	...
	/* modificadoras */
	void set_value(int f, int c, int v); // pone v en (f,c)
	void clear_value(int f, int c); // pone la celda (f,c) a VACIA
